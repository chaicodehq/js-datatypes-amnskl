...
export function generateReportCard(student) {
  ...
  const totalMarks = marksArray.reduce((sum, val) => sum + val, 0);
  ...
  const passedSubjects = subjects.filter(sub => student.marks[sub] >= 40);
  const failedSubjects = subjects.filter(sub => student.marks[sub] < 40);
